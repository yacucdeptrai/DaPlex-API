...
export class PermissionsService {
  ...
  canEditRole(authUser: AuthUserDto, targetRole: Role) {
    if (authUser.owner) return true;
    if (!authUser.roles?.length) return false;
    const minPosition = Math.min.apply(
      Math,
      authUser.roles.map((o) => o.position)
    );
    // Cannot edit higher position role
    if (minPosition >= targetRole.position) return false;
    return true;
  }

  canEditPermissions(authUser: AuthUserDto, oldPermissions: number, newPermissions: number) {
    if (authUser.owner) return true;
    if (!authUser.granted?.length) return false;
    for (let i = 0; i < authUser.granted.length; i++) {
      if (authUser.granted[i] & oldPermissions) oldPermissions = oldPermissions ^ authUser.granted[i];
      if (authUser.granted[i] & newPermissions) newPermissions = newPermissions ^ authUser.granted[i];
      if (newPermissions === 0 && oldPermissions === 0) return true;
    }
    return false;
  }
  ...
  getHighestRolePosition(user: AuthUserDto | User) {
    ...
    if (!user.roles?.length) return -1;
    return Math.min.apply(
      Math,
      user.roles.map((o: Role) => o.position)
    );
  }
}
