...
export class ImgurService {
  ...
  private async refreshToken(storage: ExternalStorage) {
    ...
    try {
      ...
      await Promise.all([this.settingsService.clearMediaBackdropCache(), this.settingsService.clearMediaPosterCache()]);
  ...
  private async uploadImage(filePath: string, fileName: string, storage: ExternalStorage, retry: number = 5, retryTimeout: number = 0) {
    for (let i = 0; i < retry; i++) {
      ...
      else file = fs.createReadStream(filePath);
      ...
      try {
        ...
        if (file instanceof fs.ReadStream) file.destroy();
      }
    }
  }
