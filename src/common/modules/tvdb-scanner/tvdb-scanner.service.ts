...
import {
  ...
  TVEpisode,
  TVSeason
} from '../../../resources/media-scanner/entities';
...
export class TvdbScannerService {
  ...
  async refreshToken() {
    if (this.tokenExpiry && this.tokenExpiry > new Date()) return;
  ...
  async search(query: string, page: number, year: number, language: string, type: SearchType) {
    try {
      ...
      const response = await firstValueFrom(
        this.httpService
          .get<PaginatedResponse<SearchResult[]>>(`${this.baseUrl}/search`, {
  ...
  async movieDetails(id: string, language: string) {
    try {
      ...
      const response = await firstValueFrom(
        this.httpService
          .get<TVDBResponse<MovieExtendedRecord>>(`${this.baseUrl}/movies/${id}/extended`, {
  ...
  async tvDetails(id: string, language: string) {
    try {
      ...
      const response = await firstValueFrom(
        this.httpService
          .get<TVDBResponse<SeriesExtendedRecord>>(`${this.baseUrl}/series/${id}/extended`, {
            params: { meta: 'translations' },
            transformRequest: (data, headers) => {
              headers.set(this.headers);
              return data;
      ...
      if (data.seasons) {
        for (let i = 0; i < data.seasons.length; i++) {
          ...
          if (s.type?.type !== 'official') continue;
          const seasonResponse = await firstValueFrom(
            this.httpService
              .get<TVDBResponse<SeasonExtendedRecord>>(`${this.baseUrl}/seasons/${s.id}/extended`, {
                params: { meta: 'translations' },
                transformRequest: (data, headers) => {
                  headers.set(this.headers);
                  return data;
  ...
  async episodeDetails(id: string, season: string, episode: string, language: string) {
    try {
      await this.refreshToken();
      const episodeQueryResponse = await firstValueFrom(
        this.httpService
          .get<TVDBResponse<SeriesExtendedRecord>>(`${this.baseUrl}/series/${id}/episodes/default`, {
  ...
  async tvImages(id: string, language: string) {
    try {
      ...
      const response = await firstValueFrom(
        this.httpService
          .get<TVDBResponse<SeriesExtendedRecord>>(`${this.baseUrl}/series/${id}/artworks`, {
  ...
  parseTVDbImageList(artworks: ArtworkBaseRecord[]) {
    ...
    if (!artworks) return result;
    // Type 2: TV Show Poster, Type 14: Movie Poster
    result.posters = artworks
      .filter((p) => [2, 14].includes(p.type))
      .map((p) => {
    ...
    result.backdrops = artworks
      .filter((p) => [3, 15].includes(p.type))
      .map((b) => {
  ...
  private mapVideos(videos: Trailer[]) {
    ...
    return videos
      .filter((v) => v.url.includes('youtube') || v.url.includes('youtu.be'))
      .map<MediaVideo>((v) => {
  ...
  private mapAltTitles(aliases: Alias[], origLangNameTranslation: Translation, tmdbAltTitles?: MediaAltTitle[]) {
    if (!aliases?.length) return tmdbAltTitles;
    return aliases
  ...
  private getUnauthorizedRetryConfig(): RetryConfig {
    return {
      ...
      delay: (error: AxiosError) => {
        if (error.isAxiosError && error.response?.status !== 401) throw error;
