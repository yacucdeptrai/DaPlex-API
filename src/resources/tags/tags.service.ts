...
export class TagsService {
  constructor(
    @InjectModel(MediaTag.name, MongooseConnection.DATABASE_A) private mediaTagModel: Model<MediaTagDocument>,
  ...
  async create(createTagDto: CreateTagDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (checkTag) throw new HttpException({ code: StatusCode.TAG_EXIST, message: 'Name has already been used' }, HttpStatus.BAD_REQUEST);
    ...
    await Promise.all([tag.save(), this.auditLogService.createLogFromBuilder(auditLog)]);
  ...
  async findOne(id: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (!tag) throw new HttpException({ code: StatusCode.TAG_NOT_FOUND, message: 'Tag not found' }, HttpStatus.NOT_FOUND);
  ...
  async update(id: bigint, updateTagDto: UpdateTagDto, headers: HeadersDto, authUser: AuthUserDto) {
    if (!Object.keys(updateTagDto).length) throw new HttpException({ code: StatusCode.EMPTY_BODY, message: 'Nothing to update' }, HttpStatus.BAD_REQUEST);
    ...
    if (translate && translate !== I18N_DEFAULT_LANGUAGE && name) {
      ...
      if (oldName !== name) {
        const checkTag = await this.mediaTagModel
    ...
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([SocketRoom.ADMIN_TAG_LIST, `${SocketRoom.ADMIN_TAG_DETAILS}:${translated._id}`]).emit(SocketMessage.REFRESH_TAGS, {
  ...
  async remove(id: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        if (!deletedTag) throw new HttpException({ code: StatusCode.TAG_NOT_FOUND, message: 'Tag not found' }, HttpStatus.NOT_FOUND);
        await Promise.all([
    ...
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([SocketRoom.ADMIN_TAG_LIST, `${SocketRoom.ADMIN_TAG_DETAILS}:${deletedTag._id}`]).emit(SocketMessage.REFRESH_TAGS, {
  ...
  async removeMany(removeTagsDto: RemoveTagsDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await session
      .withTransaction(async () => {
        const tags = await this.mediaTagModel
          .find({ _id: { $in: removeTagsDto.ids } })
          .lean()
          .session(session);
        ...
        await Promise.all(tags.map((tag) => deleteTagMediaLimit(() => this.mediaService.deleteTagMedia(tag._id, <bigint[]>(<unknown>tag.media), session))));
      })
      .finally(() => session.endSession().catch(() => {}));
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    const tagDetailsRooms = deleteTagIds.map((id) => `${SocketRoom.ADMIN_TAG_DETAILS}:${id}`);
    ioEmitter.to([SocketRoom.ADMIN_TAG_LIST, ...tagDetailsRooms]).emit(SocketMessage.REFRESH_TAGS, {
      tagIds: deleteTagIds,
      deleted: true
    });
  }
  ...
  async createMany(tags: { name: string }[], creatorId: bigint, session?: ClientSession) {
    ...
    for (let i = 0; i < tags.length; i++) {
      const createTagRes = await (<any>(
        this.mediaTagModel.findOneAndUpdate(tags[i], { $setOnInsert: { _id: await createSnowFlakeId() } }, { new: true, upsert: true, lean: true, rawResult: true, session })
      ));
      if (!createTagRes.lastErrorObject?.updatedExisting) newTagIds.push(createTagRes.value._id);
  ...
  addMediaTags(mediaId: bigint, tagIds: bigint[], session?: ClientSession) {
    if (tagIds.length) return this.mediaTagModel.updateMany({ _id: { $in: tagIds } }, { $push: { media: mediaId } }, { session });
  }

  deleteMediaTags(mediaId: bigint, tagIds: bigint[], session?: ClientSession) {
    if (tagIds.length) return this.mediaTagModel.updateMany({ _id: { $in: tagIds } }, { $pull: { media: mediaId } }, { session });
  }
