...
export class TagsService {
  ...
  async create(createTagDto: CreateTagDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (checkTag)
      throw new HttpException(
        { code: StatusCode.TAG_EXIST, message: 'Name has already been used' },
  ...
  async findOne(id: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    const tag = await this.mediaTagModel
  ...
  async update(id: bigint, updateTagDto: UpdateTagDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    const ioEmitter =
      (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter
      .to([SocketRoom.ADMIN_TAG_LIST, `${SocketRoom.ADMIN_TAG_DETAILS}:${translated._id}`])
  ...
  async remove(id: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    const ioEmitter =
      (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter
      .to([SocketRoom.ADMIN_TAG_LIST, `${SocketRoom.ADMIN_TAG_DETAILS}:${deletedTag._id}`])
      .emit(SocketMessage.REFRESH_TAGS, {
  ...
  async createMany(tags: { name: string }[], creatorId: bigint, session?: ClientSession) {
    ...
    for (let i = 0; i < tags.length; i++) {
      const createTagRes = await (<any>(
        this.mediaTagModel.findOneAndUpdate(
          tags[i],
