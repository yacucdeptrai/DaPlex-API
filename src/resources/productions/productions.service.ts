...
export class ProductionsService {
  constructor(
    @InjectModel(Production.name, MongooseConnection.DATABASE_A) private productionModel: Model<ProductionDocument>,
    ...
    await session
      .withTransaction(async () => {
        const productions = await this.productionModel
          .find({ _id: { $in: removeProductionsDto.ids } })
          .lean()
          .session(session);
  ...
  async createMany(productions: { name: string; country: string }[], creatorId: bigint, session?: ClientSession) {
    ...
    for (let i = 0; i < productions.length; i++) {
      const createProductionRes = await (<any>(
