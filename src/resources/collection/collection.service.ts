...
export class CollectionService {
  constructor(
    @InjectModel(MediaCollection.name, MongooseConnection.DATABASE_A) private collectionModel: Model<MediaCollectionDocument>,
  ...
  async create(createCollectionDto: CreateCollectionDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await Promise.all([collection.save(), this.auditLogService.createLogFromBuilder(auditLog)]);
  ...
  async findOne(id: bigint, findCollectionDto: FindCollectionDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    const collection = await this.collectionModel
      .findOne({ _id: id }, { _id: 1, name: 1, overview: 1, poster: 1, backdrop: 1, media: 1, mediaCount: 1, _translations: 1, createdAt: 1, updatedAt: 1 })
      .populate(mediaPopulation)
      .lean()
      .exec();
  ...
  async update(id: bigint, updateCollectionDto: UpdateCollectionDto, headers: HeadersDto, authUser: AuthUserDto) {
    if (!Object.keys(updateCollectionDto).length) throw new HttpException({ code: StatusCode.EMPTY_BODY, message: 'Nothing to update' }, HttpStatus.BAD_REQUEST);
    ...
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([SocketRoom.ADMIN_COLLECTION_LIST, `${SocketRoom.ADMIN_COLLECTION_DETAILS}:${collection._id}`]).emit(SocketMessage.REFRESH_COLLECTIONS, {
  ...
  async remove(id: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        if (!deletedCollection) throw new HttpException({ code: StatusCode.COLLECTION_NOT_FOUND, message: 'Collection not found' }, HttpStatus.NOT_FOUND);
        await Promise.all([
    ...
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([SocketRoom.ADMIN_COLLECTION_LIST, `${SocketRoom.ADMIN_COLLECTION_DETAILS}:${deletedCollection._id}`]).emit(SocketMessage.REFRESH_COLLECTIONS, {
    ...
    await session
      .withTransaction(async () => {
        ...
        const collections = await this.collectionModel
          .find({ _id: { $in: removeCollectionsDto.ids } })
          .lean()
          .session(session);
        ...
        await Promise.all(collections.map((collection) => deleteCollectionMediaLimit(() => this.mediaService.deleteCollectionMedia(collection._id, <bigint[]>(<unknown>collection.media), session))));
      })
      .finally(() => session.endSession().catch(() => {}));
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    const collectionDetailsRooms = deleteCollectionIds.map((id) => `${SocketRoom.ADMIN_COLLECTION_DETAILS}:${id}`);
    ioEmitter.to([SocketRoom.ADMIN_COLLECTION_LIST, ...collectionDetailsRooms]).emit(SocketMessage.REFRESH_COLLECTIONS, {
  ...
  async uploadPoster(id: bigint, file: Storage.MultipartFile, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (collection.poster) await this.deleteCollectionImage(collection.poster, CloudflareR2Container.POSTERS);
    ...
    try {
      await Promise.all([collection.save(), this.auditLogService.createLog(authUser._id, collection._id, MediaCollection.name, AuditLogType.COLLECTION_POSTER_UPDATE)]);
    } catch (e) {
  ...
  async deletePoster(id: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await Promise.all([collection.save(), this.auditLogService.createLog(authUser._id, collection._id, MediaCollection.name, AuditLogType.COLLECTION_POSTER_DELETE)]);
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([SocketRoom.ADMIN_COLLECTION_LIST, `${SocketRoom.ADMIN_COLLECTION_DETAILS}:${collection._id}`]).emit(SocketMessage.REFRESH_COLLECTIONS, {
      collectionId: collection._id
    });
  ...
  async uploadBackdrop(id: bigint, file: Storage.MultipartFile, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (collection.backdrop) await this.deleteCollectionImage(collection.backdrop, CloudflareR2Container.BACKDROPS);
    ...
    try {
      await Promise.all([collection.save(), this.auditLogService.createLog(authUser._id, collection._id, MediaCollection.name, AuditLogType.COLLECTION_BACKDROP_UPDATE)]);
    } catch (e) {
    ...
    const serializedCollection = instanceToPlain(plainToInstance(CollectionDetails, collection.toObject()));
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to(`${SocketRoom.ADMIN_COLLECTION_DETAILS}:${collection._id}`).emit(SocketMessage.REFRESH_COLLECTIONS, {
      collectionId: collection._id,
      collection: serializedCollection
    });
    return serializedCollection;
  ...
  async deleteBackdrop(id: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await Promise.all([collection.save(), this.auditLogService.createLog(authUser._id, collection._id, MediaCollection.name, AuditLogType.COLLECTION_BACKDROP_DELETE)]);
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to(`${SocketRoom.ADMIN_COLLECTION_DETAILS}:${collection._id}`).emit(SocketMessage.REFRESH_COLLECTIONS, {
      collectionId: collection._id
    });
  ...
  addMediaCollections(mediaId: bigint, collectionIds: bigint[], session?: ClientSession) {
    if (collectionIds.length) return this.collectionModel.updateMany({ _id: { $in: collectionIds } }, { $push: { media: mediaId }, $inc: { mediaCount: 1 } }, { session });
  }

  deleteMediaCollections(mediaId: bigint, collectionIds?: bigint[], session?: ClientSession) {
    if (collectionIds.length) return this.collectionModel.updateMany({ _id: { $in: collectionIds } }, { $pull: { media: mediaId }, $inc: { mediaCount: -1 } }, { session });
  }
  ...
  updateMediaCollection(mediaId: bigint, newId?: bigint, oldId?: bigint, session?: ClientSession) {
    ...
    if (oldId) writes.push({ updateOne: { filter: { _id: <any>oldId }, update: { $pull: { media: mediaId }, $inc: { mediaCount: -1 } } } });
    if (newId) writes.push({ updateOne: { filter: { _id: <any>newId }, update: { $push: { media: mediaId }, $inc: { mediaCount: 1 } } } });
  ...
  updateMediaCollections(mediaId: bigint, newIds?: bigint[], oldIds?: bigint[], session?: ClientSession) {
    const writes: Parameters<typeof this.collectionModel.bulkWrite>[0] = [];
    if (oldIds.length) writes.push({ updateMany: { filter: { _id: { $in: <any>oldIds } }, update: { $pull: { media: mediaId }, $inc: { mediaCount: -1 } } } });
    if (newIds.length) writes.push({ updateMany: { filter: { _id: { $in: <any>newIds } }, update: { $push: { media: mediaId }, $inc: { mediaCount: 1 } } } });
    return this.collectionModel.bulkWrite(writes, { session });
  }
