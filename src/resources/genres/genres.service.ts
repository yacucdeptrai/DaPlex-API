...
export class GenresService {
  constructor(
    @InjectModel(Genre.name, MongooseConnection.DATABASE_A) private genreModel: Model<GenreDocument>,
  ...
  async findAllNoPage(findGenresDto: FindGenresDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    const filters: FilterQuery<GenreDocument> = {};
  ...
  async update(id: bigint, updateGenreDto: UpdateGenreDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (translate && translate !== I18N_DEFAULT_LANGUAGE && name) {
      ...
      if (oldName !== name) {
        const checkGenre = await this.genreModel
    ...
    await session
      .withTransaction(async () => {
        ...
        const genres = await this.genreModel
          .find({ _id: { $in: removeGenresDto.ids } })
          .lean()
          .session(session);
