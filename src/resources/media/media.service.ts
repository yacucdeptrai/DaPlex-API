...
export class MediaService {
  ...
  async create(createMediaDto: CreateMediaDto, headers: HeadersDto, authUser: AuthUserDto) {
    const { type, title, originalTitle, overview, originalLang, runtime, adult, releaseDate, lastAirDate, status, inCollections, visibility, externalIds, scanner } = createMediaDto;
  ...
  async findOne(id: bigint, headers: HeadersDto, findMediaDto: FindMediaDto, authUser: AuthUserDto) {
    ...
    if (!authUser.hasPermission || !findMediaDto.includeUnprocessedEps) episodePopulation.match.visibility = MediaVisibility.PUBLIC;
    ...
    if (media.visibility === MediaVisibility.PRIVATE && !authUser.hasPermission) throw new HttpException({ code: StatusCode.MEDIA_PRIVATE, message: 'This media is private' }, HttpStatus.FORBIDDEN);
  ...
  async update(id: bigint, updateMediaDto: UpdateMediaDto, headers: HeadersDto, authUser: AuthUserDto) {
    if (!Object.keys(updateMediaDto).length) throw new HttpException({ code: StatusCode.EMPTY_BODY, message: 'Nothing to update' }, HttpStatus.BAD_REQUEST);
    ...
    if (updateMediaDto.translate && updateMediaDto.translate !== I18N_DEFAULT_LANGUAGE) {
      ...
      await session
        .withTransaction(async () => {
          ...
          if (updateMediaDto.lastAirDate !== undefined && media.type === MediaType.TV) media.tv.lastAirDate = updateMediaDto.lastAirDate;
          if (updateMediaDto.externalIds) {
            if (updateMediaDto.externalIds.imdb !== undefined) media.set('externalIds.imdb', updateMediaDto.externalIds.imdb);
            if (updateMediaDto.externalIds.tmdb !== undefined) media.set('externalIds.tmdb', updateMediaDto.externalIds.tmdb);
            if (updateMediaDto.externalIds.aniList !== undefined) media.set('externalIds.aniList', updateMediaDto.externalIds.aniList);
            if (updateMediaDto.externalIds.mal !== undefined) media.set('externalIds.mal', updateMediaDto.externalIds.mal);
          }
          if (updateMediaDto.scanner) {
            if (updateMediaDto.scanner.enabled != undefined) media.set('scanner.enabled', updateMediaDto.scanner.enabled);
            if (updateMediaDto.scanner.tvSeason !== undefined && media.type === MediaType.TV) media.set('scanner.tvSeason', updateMediaDto.scanner.tvSeason);
          }
    ...
    await Promise.all([this.auditLogService.createLogFromBuilder(auditLog), this.localCacheService.del(`${CachePrefix.MEDIA_FIND_FILTER_RELATED}:${id}`)]);
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${translated._id}`]).emit(SocketMessage.REFRESH_MEDIA, {
  ...
  async remove(id: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        if (!deletedMedia) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
        await Promise.all([
        ...
        if (deletedMedia.type === MediaType.MOVIE) {
          ...
          await Promise.all(deletedMedia.tv.episodes.map((episodeId) => deleteEpisodeLimit(() => this.deleteEpisodeById(<bigint>(<unknown>episodeId), session))));
        }
        await this.auditLogService.createLog(authUser._id, deletedMedia._id, Media.name, AuditLogType.MEDIA_DELETE);
      })
      .finally(() => session.endSession().catch(() => {}));
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${deletedMedia._id}`]).emit(SocketMessage.REFRESH_MEDIA, {
  ...
  async addMediaVideo(id: bigint, addMediaVideoDto: AddMediaVideoDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (!urlMatch || urlMatch[1].length !== 11) throw new HttpException({ code: StatusCode.INVALID_YOUTUBE_URL, message: 'Invalid YouTube url' }, HttpStatus.BAD_REQUEST);
    ...
    if (!media) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
    if (media.videos.find((v) => v.key === urlMatch[1])) throw new HttpException({ code: StatusCode.MEDIA_VIDEO_EXIST, message: 'This video has already been added' }, HttpStatus.BAD_REQUEST);
  ...
  async deleteMediaVideo(id: bigint, videoId: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    const media = await this.mediaModel
      .findOneAndUpdate({ _id: id, videos: { $elemMatch: { _id: videoId } } }, { $pull: { videos: { _id: videoId } } }, { new: true, timestamps: false })
      .lean()
      .exec();
    if (!media) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
  ...
  async deleteMediaVideos(id: bigint, deleteMediaVideosDto: DeleteMediaVideosDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    const deletedMedia = await this.mediaModel
      .findOneAndUpdate({ _id: id }, { $pull: { videos: { _id: { $in: deleteVideoIds } } } }, { new: true, timestamps: false })
      .lean()
      .exec();
  ...
  async uploadMediaPoster(id: bigint, file: Storage.MultipartFile, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    media.poster = poster;
    try {
      await Promise.all([media.save({ timestamps: false }), this.auditLogService.createLog(authUser._id, media._id, Media.name, AuditLogType.MEDIA_POSTER_UPDATE)]);
    } catch (e) {
    ...
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${media._id}`]).emit(SocketMessage.REFRESH_MEDIA, {
      mediaId: media._id,
      media: serializedMedia
    });
    return serializedMedia;
  ...
  async deleteMediaPoster(id: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await Promise.all([media.save({ timestamps: false }), this.auditLogService.createLog(authUser._id, media._id, Media.name, AuditLogType.MEDIA_POSTER_DELETE)]);
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${media._id}`]).emit(SocketMessage.REFRESH_MEDIA, {
      mediaId: media._id
    });
  ...
  async uploadMediaBackdrop(id: bigint, file: Storage.MultipartFile, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    try {
      await Promise.all([media.save({ timestamps: false }), this.auditLogService.createLog(authUser._id, media._id, Media.name, AuditLogType.MEDIA_BACKDROP_UPDATE)]);
    } catch (e) {
  ...
  async deleteMediaBackdrop(id: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await Promise.all([media.save({ timestamps: false }), this.auditLogService.createLog(authUser._id, media._id, Media.name, AuditLogType.MEDIA_BACKDROP_DELETE)]);
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to(`${SocketRoom.ADMIN_MEDIA_DETAILS}:${media._id}`).emit(SocketMessage.REFRESH_MEDIA, {
      mediaId: media._id
    });
  ...
  async uploadMovieSubtitle(id: bigint, file: Storage.MultipartFile, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (media.movie.subtitles?.length) {
      ...
      if (subtitle) throw new HttpException({ code: StatusCode.SUBTITLE_EXIST, message: 'Subtitle with this language has already been added' }, HttpStatus.BAD_REQUEST);
    }
  ...
  async deleteMovieSubtitles(id: bigint, deleteMediaSubtitlesDto: DeleteMediaSubtitlesDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    const updatedMedia = await this.mediaModel
      .findOneAndUpdate({ _id: id }, { $pull: { 'movie.subtitles': { _id: { $in: deleteSubtitles.map((s) => s._id) } } } }, { new: true, timestamps: false })
  ...
  async uploadMovieSource(id: bigint, addMediaSourceDto: AddMediaSourceDto, authUser: AuthUserDto) {
    ...
    if (!media) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
    if (media.movie.source) throw new HttpException({ code: StatusCode.MEDIA_SOURCE_EXIST, message: 'Source has already been added' }, HttpStatus.BAD_REQUEST);
  ...
  async addLinkedMovieSource(id: bigint, addLinkedMediaSourceDto: AddLinkedMediaSourceDto, baseUrl: string, authUser: AuthUserDto) {
    ...
    this.wsAdminGateway.server.to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${media._id}`]).emit(SocketMessage.SAVE_MOVIE_SOURCE, {
      mediaId: media._id
    });
  }
  ...
  async encodeMovieSource(id: bigint, encodeMediaSourceDto: EncodeMediaSourceDto, baseUrl: string, authUser: AuthUserDto) {
    ...
    if (!media) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
    if (!media.movie.source) throw new HttpException({ code: StatusCode.MEDIA_SOURCE_NOT_FOUND, message: 'Media source not found' }, HttpStatus.NOT_FOUND);
    ...
    const updateQuery: UpdateQuery<MediaStorageDocument> = encodeMediaSourceDto.options
      ...
      : {};
    const uploadedSource = await this.mediaStorageModel.findOneAndUpdate({ _id: media.movie.source }, updateQuery, { new: true }).populate('storage').lean().exec();
    if (!uploadedSource) throw new HttpException({ code: StatusCode.MEDIA_SOURCE_NOT_FOUND, message: 'Media source not found' }, HttpStatus.NOT_FOUND);
    /*
    ...
    */
    const streamSettings = await this.settingsService.findStreamSettings();
    const targetVideoCodecs = uploadedSource.options?.videoCodecs || (streamSettings.defaultVideoCodecs !== STREAM_CODECS[0] ? streamSettings.defaultVideoCodecs : null);
  ...
  async saveMovieSource(id: bigint, sessionId: bigint, saveMediaSourceDto: SaveMediaSourceDto, baseUrl: string, authUser: AuthUserDto) {
    const media = await this.mediaModel.findOne({ _id: id, type: MediaType.MOVIE }, { _id: 1, movie: 1, pStatus: 1 }).exec();
    if (!media) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
    if (media.movie.source) throw new HttpException({ code: StatusCode.MEDIA_SOURCE_EXIST, message: 'Source has already been added' }, HttpStatus.CONFLICT);
    const uploadSession = await this.driveSessionModel
      ...
      .lean()
      .exec();
    if (!uploadSession) throw new HttpException({ code: StatusCode.DRIVE_SESSION_NOT_FOUND, message: 'Upload session not found' }, HttpStatus.NOT_FOUND);
    let fileInfo: any;
  ...
  async deleteMovieSource(id: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (!media) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
    if (media.movie.status === MediaSourceStatus.PENDING) throw new HttpException({ code: StatusCode.MEDIA_SOURCE_NOT_FOUND, message: 'Media source not found' }, HttpStatus.NOT_FOUND);
    const session = await this.mongooseConnection.startSession();
    await session
      .withTransaction(async () => {
        ...
        await Promise.all([media.save({ session, timestamps: false }), this.auditLogService.createLogFromBuilder(auditLog)]);
      })
      .finally(() => session.endSession().catch(() => {}));
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${media._id}`]).emit(SocketMessage.DELETE_MOVIE_SOURCE, {
      mediaId: media._id
    });
  }
  ...
  updateMediaSourceData(mediaQueueResultDto: MediaQueueResultDto) {
    ...
    if (mediaQueueResultDto.episode) {
      ...
      updatePromises.push(this.tvEpisodeModel.updateOne({ _id: mediaQueueResultDto.episode, media: mediaQueueResultDto.media }, { $set: { runtime: mediaQueueResultDto.progress.runtime } }).exec());
    }
    updatePromises.push(this.mediaModel.updateOne({ _id: mediaQueueResultDto.media, runtime: null }, { $set: { runtime: mediaQueueResultDto.progress.runtime } }, { timestamps: false }).exec());
    updatePromises.push(this.mediaStorageModel.updateOne(updateStorageFilters, { $set: { quality: mediaQueueResultDto.progress.quality } }).exec());
  ...
  async addMovieStreamManifest(mediaQueueResultDto: MediaQueueResultDto) {
    ...
    await session
      .withTransaction(async () => {
        media = await this.mediaModel.findOne({ _id: mediaQueueResultDto.media, type: MediaType.MOVIE }, { _id: 1, movie: 1, pStatus: 1 }, { session });
        if (!media) return;
        source = await this.mediaStorageModel.findOne({ _id: mediaQueueResultDto._id }, {}, { session });
        if (!source) return;
        const fileMimeType = mimeTypes.lookup(mediaQueueResultDto.progress.fileName) || 'application/octet-stream';
        ...
        if (media.pStatus !== MediaPStatus.DONE) {
          ...
          this.wsAdminGateway.server.to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${media._id}`]).emit(SocketMessage.ADD_MOVIE_STREAM, {
            mediaId: media._id
          });
        }
        await source.save({ session });
  ...
  async handleMovieStreamQueueDone(jobId: number | string, mediaQueueResultDto: MediaQueueResultDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        // Replace old streams
        if (mediaQueueResultDto.replaceStreams?.length) await this.deleteMediaStreams(mediaQueueResultDto.replaceStreams, <bigint>(<unknown>media.movie.source), session);
      })
      .finally(() => session.endSession().catch(() => {}));
    if (!mediaQueueResultDto.isPrimary) return;
    this.wsAdminGateway.server.to(`${SocketRoom.USER_ID}:${mediaQueueResultDto._id}`).emit(SocketMessage.MEDIA_PROCESSING_SUCCESS, {
      mediaId: mediaQueueResultDto.media
    });
    this.wsAdminGateway.server.to(SocketRoom.ADMIN_MEDIA_LIST).to(`${SocketRoom.ADMIN_MEDIA_DETAILS}:${mediaQueueResultDto.media}`).emit(SocketMessage.REFRESH_MEDIA, {
      mediaId: mediaQueueResultDto.media
    });
    /*
  ...
  async handleMovieStreamQueueCancel(jobId: number | string, mediaQueueResultDto: MediaQueueResultDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        if (mediaQueueResultDto.keepStreams) updateSetQuery = { 'movie.status': MediaSourceStatus.DONE, pStatus: MediaPStatus.DONE };
        const media = await this.mediaModel
          .findOneAndUpdate(
    ...
    await session
      .withTransaction(async () => {
        ...
        if (mediaQueueResultDto.codec === VideoCodec.H264) {
          streamsByCodec = source.streams.filter((s) => s.type === MediaStorageType.STREAM_AUDIO || (MediaStorageType.STREAM_VIDEO && s.codec === VideoCodec.H264));
        } else {
      ...
      await session
        .withTransaction(async () => {
          if (mediaQueueResultDto.isPrimary) {
            ...
            if (source?.streams) {
              const streamsByCodec = source.streams.filter((s) => s.type === MediaStorageType.STREAM_VIDEO && s.codec === mediaQueueResultDto.codec);
          ...
          this.wsAdminGateway.server.to(`${SocketRoom.USER_ID}:${mediaQueueResultDto.user}`).emit(SocketMessage.MEDIA_PROCESSING_FAILURE, {
            mediaId: media._id
          });
          this.wsAdminGateway.server.to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${media._id}`]).emit(SocketMessage.REFRESH_MEDIA, {
            mediaId: media._id
          });
        })
        .finally(() => session.endSession().catch(() => {}));
  ...
  async findAllMovieStreams(id: bigint, findMediaStreamsDto: FindMediaStreamsDto, authUser: AuthUserDto) {
    ...
    const media = await this.mediaModel
      .findOneAndUpdate({ _id: id, type: MediaType.MOVIE }, { $inc: { views: incViews, dailyViews: incViews, weeklyViews: incViews, monthlyViews: incViews } }, { timestamps: false })
      .select({ _id: 1, movie: 1, pStatus: 1, visibility: 1 })
      .populate([{ path: 'movie.source', populate: { path: 'storage', select: { _id: 1, kind: 1, folderId: 1, publicUrl: 1, secondPublicUrl: 1 } } }])
      .lean()
      .exec();
    if (!media) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
    if (media.visibility === MediaVisibility.PRIVATE && !authUser.hasPermission) throw new HttpException({ code: StatusCode.MEDIA_PRIVATE, message: 'This media is private' }, HttpStatus.FORBIDDEN);
    if (media.pStatus !== MediaPStatus.DONE) throw new HttpException({ code: StatusCode.MOVIE_NOT_READY, message: 'Movie is not ready' }, HttpStatus.NOT_FOUND);
    if (!media.movie.source.streams?.length) throw new HttpException({ code: StatusCode.MEDIA_STREAM_NOT_FOUND, message: 'Media stream not found' }, HttpStatus.NOT_FOUND);
  ...
  async addMovieChapter(id: bigint, addMediaChapterDto: AddMediaChapterDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        auditLog.getChangesFrom(media);
        await media.save({ session, timestamps: false });
        await Promise.all([this.chapterTypeService.addMovieChapterType(id, <bigint>(<unknown>chapter.type), session), this.auditLogService.createLogFromBuilder(auditLog)]);
      })
      .finally(() => session.endSession().catch(() => {}));
  ...
  async updateMovieChapter(id: bigint, chapterId: bigint, updateMediaChapterDto: UpdateMediaChapterDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (chapterIndex === -1) throw new HttpException({ code: StatusCode.CHAPTER_NOT_FOUND, message: 'Chapter not found' }, HttpStatus.NOT_FOUND);
    ...
    await session
      .withTransaction(async () => {
        ...
        if (updateMediaChapterDto.type != undefined) {
          ...
          await this.chapterTypeService.updateMovieChapterType(id, <bigint>(<unknown>targetChapter.type), updateMediaChapterDto.type, session);
  ...
  async deleteMovieChapter(id: bigint, chapterId: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        if (!chapter) throw new HttpException({ code: StatusCode.CHAPTER_NOT_FOUND, message: 'Chapter not found' }, HttpStatus.NOT_FOUND);
  ...
  async deleteMovieChapters(id: bigint, deleteMediaChaptersDto: DeleteMediaChaptersDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await session
      .withTransaction(async () => {
        const updatedMedia = await this.mediaModel.findOneAndUpdate({ _id: id }, { $pull: { 'movie.chapters': { _id: { $in: deleteChapterIds } } } }, { new: true, session, timestamps: false }).lean();
        ...
        await Promise.all([this.chapterTypeService.deleteMovieChapterTypes(id, deleteChapterTypes, session), this.auditLogService.createLogFromBuilder(auditLog)]);
      })
      .finally(() => session.endSession().catch(() => {}));
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
  ...
  async addTVEpisode(id: bigint, addTVEpisodeDto: AddTVEpisodeDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (episodeExist) throw new HttpException({ code: StatusCode.EPISODE_NUMBER_EXIST, message: 'Episode number has already been used' }, HttpStatus.BAD_REQUEST);
    let media: MediaDocument;
    ...
    await session
      .withTransaction(async () => {
        ...
        // Save media
        await Promise.all([media.updateOne(media.getChanges(), { session, timestamps: false }), this.auditLogService.createLogFromBuilder(auditLog)]);
      })
      .finally(() => session.endSession().catch(() => {}));
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${media._id}`]).emit(SocketMessage.REFRESH_MEDIA, {
  ...
  async findAllTVEpisodes(id: bigint, findEpisodesDto: FindTVEpisodesDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    const media = await this.mediaModel.findOne({ _id: id, type: MediaType.TV }, { tv: 1 }).populate(population).lean().exec();
    if (!media) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
    if (media.visibility === MediaVisibility.PRIVATE && !authUser.hasPermission) throw new HttpException({ code: StatusCode.MEDIA_PRIVATE, message: 'This media is private' }, HttpStatus.FORBIDDEN);
  ...
  async findOneTVEpisode(id: bigint, episodeId: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (!episode) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
  ...
  async updateTVEpisode(id: bigint, episodeId: bigint, updateTVEpisodeDto: UpdateTVEpisodeDto, headers: HeadersDto, authUser: AuthUserDto) {
    if (!Object.keys(updateTVEpisodeDto).length) throw new HttpException({ code: StatusCode.EMPTY_BODY, message: 'Nothing to update' }, HttpStatus.BAD_REQUEST);
      ...
      await session
        .withTransaction(async () => {
          ...
          if (updateTVEpisodeDto.epNumber != undefined && updateTVEpisodeDto.epNumber !== episode.epNumber) {
            const episodeExist = await this.tvEpisodeModel.findOne({ media: id, epNumber: updateTVEpisodeDto.epNumber }).lean().exec();
            if (episodeExist) throw new HttpException({ code: StatusCode.EPISODE_NUMBER_EXIST, message: 'Episode number has already been used' }, HttpStatus.BAD_REQUEST);
          ...
          if (episode.isModified('visibility') || episode.isModified('epNumber')) {
            ...
            if (media.tv.pLastEpisode == undefined) {
              if (episode.visibility === MediaVisibility.PUBLIC && episode.pStatus === MediaPStatus.DONE) media.tv.pLastEpisode = episode._id;
            } else {
    ...
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([`${SocketRoom.ADMIN_MEDIA_DETAILS}:${id}`, `${SocketRoom.ADMIN_EPISODE_DETAILS}:${episode._id}`]).emit(SocketMessage.REFRESH_TV_EPISODE, {
      mediaId: id,
    ...
    await session
      .withTransaction(async () => {
        ...
        if (episode._id === media.tv.pLastEpisode) {
          const publicLastEpisode = media.tv.lastEpisode === media.tv.pLastEpisode ? lastEpisode : await this.findLastEpisode(id, episode._id, true, true);
          media.tv.pLastEpisode = <any>publicLastEpisode?._id;
        }
        await Promise.all([media.save({ session, timestamps: false }), this.auditLogService.createLog(authUser._id, episode._id, TVEpisode.name, AuditLogType.EPISODE_DELETE)]);
      })
      .finally(() => session.endSession().catch(() => {}));
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([`${SocketRoom.ADMIN_MEDIA_DETAILS}:${id}`, `${SocketRoom.ADMIN_EPISODE_DETAILS}:${episodeId}`]).emit(SocketMessage.REFRESH_TV_EPISODE, {
      mediaId: id,
      episodeId: episodeId,
      deleted: true
    });
  ...
  async uploadTVEpisodeStill(id: bigint, episodeId: bigint, file: Storage.MultipartFile, headers: HeadersDto, authUser: AuthUserDto) {
    const episode = await this.tvEpisodeModel.findOne({ _id: episodeId, media: id }, { epNumber: 1, name: 1, overview: 1, runtime: 1, still: 1, airDate: 1, visibility: 1 }).exec();
    if (!episode) throw new HttpException({ code: StatusCode.EPISODE_NOT_FOUND, message: 'Episode not found' }, HttpStatus.NOT_FOUND);
    ...
    await session
      .withTransaction(async () => {
        ...
        episode.still = still;
        try {
          await Promise.all([episode.save({ session }), this.auditLogService.createLog(authUser._id, episode._id, TVEpisode.name, AuditLogType.EPISODE_STILL_UPDATE)]);
        } catch (e) {
  ...
  async deleteTVEpisodeStill(id: bigint, episodeId: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        await Promise.all([episode.save({ session }), this.auditLogService.createLog(authUser._id, episode._id, TVEpisode.name, AuditLogType.EPISODE_STILL_DELETE)]);
      })
      .finally(() => session.endSession().catch(() => {}));
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
    ioEmitter.to([`${SocketRoom.ADMIN_MEDIA_DETAILS}:${id}`, `${SocketRoom.ADMIN_EPISODE_DETAILS}:${episodeId}`]).emit(SocketMessage.REFRESH_TV_EPISODE, {
  ...
  async deleteTVEpisodeSubtitles(id: bigint, episodeId: bigint, deleteMediaSubtitlesDto: DeleteMediaSubtitlesDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    const updatedEpisode = await this.tvEpisodeModel
      .findOneAndUpdate({ _id: episodeId, media: <any>id }, { $pull: { subtitles: { _id: { $in: deleteSubtitles.map((s) => s._id) } } } }, { new: true })
      .select({ subtitles: 1 })
      .lean()
      .exec();
  ...
  async uploadTVEpisodeSource(id: bigint, episodeId: bigint, addMediaSourceDto: AddMediaSourceDto, authUser: AuthUserDto) {
    ...
    if (!episode) throw new HttpException({ code: StatusCode.EPISODE_NOT_FOUND, message: 'Episode not found' }, HttpStatus.NOT_FOUND);
    if (episode.source) throw new HttpException({ code: StatusCode.MEDIA_SOURCE_EXIST, message: 'Source has already been added' }, HttpStatus.BAD_REQUEST);
    return this.createUploadSourceSession(addMediaSourceDto, authUser._id);
  }
  ...
  async addLinkedTVEpisodeSource(id: bigint, episodeId: bigint, addLinkedMediaSourceDto: AddLinkedMediaSourceDto, baseUrl: string, authUser: AuthUserDto) {
    ...
    if (!media) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
    const episode = await this.tvEpisodeModel.findOne({ _id: episodeId, media: id }, { source: 1, status: 1, pStatus: 1, tJobs: 1 }).exec();
    if (!episode) throw new HttpException({ code: StatusCode.EPISODE_NOT_FOUND, message: 'Episode not found' }, HttpStatus.NOT_FOUND);
    if (episode.source) throw new HttpException({ code: StatusCode.MEDIA_SOURCE_EXIST, message: 'Source has already been added' }, HttpStatus.BAD_REQUEST);
    ...
    await media.save({ timestamps: false });
    await episode.save();
    this.wsAdminGateway.server.to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${id}`, `${SocketRoom.ADMIN_EPISODE_DETAILS}:${episodeId}`]).emit(SocketMessage.SAVE_TV_SOURCE, {
      mediaId: media._id,
      episodeId: episode._id
    });
  }
  ...
  async encodeTVEpisodeSource(id: bigint, episodeId: bigint, encodeMediaSourceDto: EncodeMediaSourceDto, baseUrl: string, authUser: AuthUserDto) {
    ...
    if (!episode) throw new HttpException({ code: StatusCode.EPISODE_NOT_FOUND, message: 'Episode not found' }, HttpStatus.NOT_FOUND);
    if (!episode.source) throw new HttpException({ code: StatusCode.MEDIA_SOURCE_NOT_FOUND, message: 'Media source not found' }, HttpStatus.NOT_FOUND);
    if (episode.status !== MediaSourceStatus.DONE) throw new HttpException({ code: StatusCode.EPISODE_ENCODING_UNAVAILABLE, message: 'This feature is currently not available' }, HttpStatus.NOT_FOUND);
    const sourceAdvancedOptions = encodeMediaSourceDto.options || {};
    // Save options to database
    const { selectAudioTracks, extraAudioTracks, forceVideoQuality, h264Tune, queuePriority, videoCodecs, overrideSettings, audioOnly, videoOnly } = sourceAdvancedOptions;
    const updateQuery: UpdateQuery<MediaStorageDocument> = encodeMediaSourceDto.options
      ? { $set: { options: { selectAudioTracks, extraAudioTracks, forceVideoQuality, h264Tune, queuePriority, videoCodecs, overrideSettings } } }
      : {};
    const uploadedSource = await this.mediaStorageModel.findOneAndUpdate({ _id: episode.source }, updateQuery, { new: true }).populate('storage').lean().exec();
    if (!uploadedSource) throw new HttpException({ code: StatusCode.MEDIA_SOURCE_NOT_FOUND, message: 'Media source not found' }, HttpStatus.NOT_FOUND);
    const streamSettings = await this.settingsService.findStreamSettings();
    const targetVideoCodecs = uploadedSource.options?.videoCodecs || (streamSettings.defaultVideoCodecs !== STREAM_CODECS[0] ? streamSettings.defaultVideoCodecs : null);
    const replaceStreams = this.findReplaceStreams(uploadedSource, targetVideoCodecs, audioOnly, videoOnly);
    ...
    await episode.save();
    this.wsAdminGateway.server.to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${episode._id}`]).emit(SocketMessage.SAVE_MOVIE_SOURCE, {
      mediaId: episode._id
    });
  }
  ...
  async saveTVEpisodeSource(id: bigint, episodeId: bigint, sessionId: bigint, saveMediaSourceDto: SaveMediaSourceDto, baseUrl: string, authUser: AuthUserDto) {
    const media = await this.mediaModel.findOne({ _id: id, type: MediaType.TV }, { _id: 1, pStatus: 1 }).exec();
    if (!media) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
    const episode = await this.tvEpisodeModel.findOne({ _id: episodeId, media: id }, { _id: 1, source: 1, status: 1, tJobs: 1 }).exec();
    if (!episode) throw new HttpException({ code: StatusCode.EPISODE_NOT_FOUND, message: 'Episode not found' }, HttpStatus.NOT_FOUND);
    if (episode.source) throw new HttpException({ code: StatusCode.MEDIA_SOURCE_EXIST, message: 'Source has already been added' }, HttpStatus.CONFLICT);
    const uploadSession = await this.driveSessionModel.findOne({ _id: sessionId, user: authUser._id }).populate('storage').lean().exec();
    if (!uploadSession) throw new HttpException({ code: StatusCode.DRIVE_SESSION_NOT_FOUND, message: 'Upload session not found' }, HttpStatus.NOT_FOUND);
    let fileInfo: any;
    if (uploadSession.storage.kind === CloudStorage.FILER) {
      fileInfo = await this.filerService.findId(saveMediaSourceDto.fileId, uploadSession.storage);
  ...
  async deleteTVEpisodeSource(id: bigint, episodeId: bigint, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (!media) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
    const episode = await this.tvEpisodeModel.findOne({ _id: episodeId, media: id }, { _id: 1, source: 1, streams: 1, status: 1, tJobs: 1 }).exec();
    if (!episode) throw new HttpException({ code: StatusCode.EPISODE_NOT_FOUND, message: 'Episode not found' }, HttpStatus.NOT_FOUND);
    const session = await this.mongooseConnection.startSession();
    await session
      .withTransaction(async () => {
        ...
        if (episode._id === media.tv.pLastEpisode) {
          const publicLastEpisode = media.tv.pLastEpisode === media.tv.lastEpisode ? lastEpisode : await this.findLastEpisode(id, episode._id, true, true);
          media.tv.pLastEpisode = <any>publicLastEpisode?._id;
        }
  ...
  async addTVEpisodeStreamManifest(mediaQueueResultDto: MediaQueueResultDto) {
    ...
    await session
      .withTransaction(async () => {
        media = await this.mediaModel.findOne({ _id: mediaQueueResultDto.media, type: MediaType.TV }, { _id: 1, tv: 1, pStatus: 1 }, { session }).populate([
        ...
        if (episode.pStatus !== MediaPStatus.DONE) {
          ...
          if (episode.visibility === MediaVisibility.PUBLIC && (!media.tv.pLastEpisode || media.tv.pLastEpisode.epNumber < episode.epNumber)) {
        ...
        if (media.pStatus !== MediaPStatus.DONE) {
          ...
          this.wsAdminGateway.server
            .to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${media._id}`, `${SocketRoom.ADMIN_EPISODE_DETAILS}:${episode._id}`])
  ...
  async handleTVEpisodeStreamQueueDone(jobId: number | string, mediaQueueResultDto: MediaQueueResultDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        episode = await this.tvEpisodeModel.findOneAndUpdate({ _id: mediaQueueResultDto.episode, media: mediaQueueResultDto.media }, updateQuery, { session, lean: true });
        if (mediaQueueResultDto.replaceStreams?.length) await this.deleteMediaStreams(mediaQueueResultDto.replaceStreams, <bigint>(<unknown>episode.source), session);
      })
      .finally(() => session.endSession().catch(() => {}));
    if (!mediaQueueResultDto.isPrimary) return;
    this.wsAdminGateway.server.to(`${SocketRoom.USER_ID}:${mediaQueueResultDto._id}`).emit(SocketMessage.MEDIA_PROCESSING_SUCCESS, {
  ...
  async handleTVEpisodeStreamQueueCancel(jobId: number | string, mediaQueueResultDto: MediaQueueResultDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        if (mediaQueueResultDto.keepStreams) updateSetQuery = { status: MediaSourceStatus.DONE, pStatus: MediaPStatus.DONE };
        episode = await this.tvEpisodeModel
          .findOneAndUpdate({ _id: mediaQueueResultDto.episode, media: mediaQueueResultDto.media }, { $pull: { tJobs: jobId }, $set: updateSetQuery }, { session, lean: true })
  ...
  async handleTVEpisodeStreamQueueError(jobId: number | string, mediaQueueResultDto: MediaQueueResultDto) {
    ...
    if (episode && <bigint>(<unknown>episode.source) === mediaQueueResultDto._id) {
      ...
      this.wsAdminGateway.server
        .to([SocketRoom.ADMIN_MEDIA_LIST, `${SocketRoom.ADMIN_MEDIA_DETAILS}:${mediaQueueResultDto.media}`, `${SocketRoom.ADMIN_EPISODE_DETAILS}:${episode._id}`])
        .emit(SocketMessage.REFRESH_MEDIA, {
          mediaId: mediaQueueResultDto.media
        });
    }
  ...
  async findAllTVEpisodeStreams(id: bigint, epNumber: number, findMediaStreamsDto: FindMediaStreamsDto, authUser: AuthUserDto) {
    const incViews = findMediaStreamsDto.preview && authUser.hasPermission ? 0 : 1;
    const media = await this.mediaModel
      .findOneAndUpdate({ _id: id, type: MediaType.TV }, { $inc: { views: incViews, dailyViews: incViews, weeklyViews: incViews, monthlyViews: incViews } }, { timestamps: false })
      .lean()
      .exec();
    if (!media) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
    if (media.visibility === MediaVisibility.PRIVATE && !authUser.hasPermission) throw new HttpException({ code: StatusCode.MEDIA_PRIVATE, message: 'This media is private' }, HttpStatus.FORBIDDEN);
    if (media.pStatus !== MediaPStatus.DONE) throw new HttpException({ code: StatusCode.TV_NOT_READY, message: 'TV Show is not ready' }, HttpStatus.NOT_FOUND);
    const episode = await this.tvEpisodeModel
      .findOneAndUpdate({ media: id, epNumber: epNumber }, { $inc: { views: 1 } }, { timestamps: false })
      .populate([{ path: 'source', populate: { path: 'storage', select: { _id: 1, kind: 1, folderId: 1, publicUrl: 1, secondPublicUrl: 1 } } }])
      .lean()
      .exec();
    if (!episode) throw new HttpException({ code: StatusCode.EPISODE_NOT_FOUND, message: 'Episode not found' }, HttpStatus.NOT_FOUND);
    if (episode.visibility === MediaVisibility.PRIVATE && !authUser.hasPermission)
      throw new HttpException({ code: StatusCode.EPISODE_PRIVATE, message: 'This episode is private' }, HttpStatus.FORBIDDEN);
    if (!episode.source.streams?.length) throw new HttpException({ code: StatusCode.MEDIA_STREAM_NOT_FOUND, message: 'Media stream not found' }, HttpStatus.NOT_FOUND);
    ...
    const storage = {
      ...
      secondPublicUrl: episode.source.storage.secondPublicUrl
        ? this.resolveStoragePublicUrl(episode.source.storage.kind, episode.source.storage.secondPublicUrl, episode.source.storage.folderId)
        : undefined
    };
    return plainToInstance(MediaStream, {
      _id: media._id,
  ...
  async addTVEpisodeChapter(id: bigint, episodeId: bigint, addMediaChapterDto: AddMediaChapterDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        await episode.save({ session });
        await Promise.all([this.chapterTypeService.addTVEpisodeChapterType(episodeId, <bigint>(<unknown>chapter.type), session), this.auditLogService.createLogFromBuilder(auditLog)]);
      })
      .finally(() => session.endSession().catch(() => {}));
    const chapterType = await this.chapterTypeService.findById(addMediaChapterDto.type);
    const populatedChapter: MediaChapter = { ...chapter, type: chapterType };
  ...
  async updateTVEpisodeChapter(id: bigint, episodeId: bigint, chapterId: bigint, updateMediaChapterDto: UpdateMediaChapterDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        if (updateMediaChapterDto.type != undefined) {
          await this.validateChapterType(updateMediaChapterDto.type);
          await this.chapterTypeService.updateTVEpisodeChapterType(episodeId, <bigint>(<unknown>targetChapter.type), updateMediaChapterDto.type, session);
          targetChapter.type = <any>updateMediaChapterDto.type;
        }
        if (updateMediaChapterDto.start != undefined) {
          targetChapter.start = updateMediaChapterDto.start;
  ...
  async deleteTVEpisodeChapters(id: bigint, episodeId: bigint, deleteMediaChaptersDto: DeleteMediaChaptersDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    await session
      .withTransaction(async () => {
        const updatedEpisode = await this.tvEpisodeModel
          .findOneAndUpdate({ _id: episodeId, media: <any>id }, { $pull: { chapters: { _id: { $in: deleteChapterIds } } } }, { new: true, session })
          .lean();
        const auditLog = new AuditLogBuilder(authUser._id, updatedEpisode._id, Media.name, AuditLogType.EPISODE_CHAPTER_DELETE);
        deleteChapterIds.forEach((id) => {
          auditLog.appendChange('_id', undefined, id);
        });
        await Promise.all([this.chapterTypeService.deleteTVEpisodeChapterTypes(episodeId, deleteChapterTypes, session), this.auditLogService.createLogFromBuilder(auditLog)]);
      })
      .finally(() => session.endSession().catch(() => {}));
    const ioEmitter = (headers.socketId && this.wsAdminGateway.server.sockets.get(headers.socketId)) || this.wsAdminGateway.server;
  ...
  private async createFindAllParams(paginateMediaDto: OffsetPageMediaDto | CursorPageMediaDto, hasPermission: boolean): Promise<[{ [key: string]: number }, { [key: string]: any }]> {
    ...
    const { adult, type, originalLang, year, genres, tags, genreMatch, tagMatch, excludeIds, preset, presetParams, includeHidden, includeUnprocessed } = paginateMediaDto;
    ...
    if (presetParams?.length) {
      if (preset === 'related') {
        ...
        filters.$or = await this.localCacheService.wrap(
          ...
          async () => {
            const refMedia = await this.mediaModel.findOne({ _id: refMediaId }, { genres: 1, studios: 1, producers: 1, tags: 1 }).lean().exec();
            return [{ genres: { $in: refMedia.genres } }, { producers: { $in: refMedia.producers } }, { studios: { $in: refMedia.studios } }, { tags: { $in: refMedia.tags } }];
          },
          1_800_000
        );
  ...
  private async findOrCreateGenres(genres: string[], creatorId: bigint, session: ClientSession) {
    ...
    for (let i = 0; i < genres.length; i++) {
      if (genres[i].startsWith('create:')) {
        ...
        if (!name) throw new HttpException({ code: StatusCode.IS_NOT_EMPTY, message: 'Genre name must not be empty' }, HttpStatus.BAD_REQUEST);
        if (name.length > 32) throw new HttpException({ code: StatusCode.MAX_LENGTH, message: 'Genre name must not be longer than 32 characters' }, HttpStatus.BAD_REQUEST);
        newGenres.push({ name });
      } else {
        try {
    ...
    if (genreCount !== existingGenreIds.length) throw new HttpException({ code: StatusCode.GENRES_NOT_FOUND, message: 'Cannot find all the required genres' }, HttpStatus.BAD_REQUEST);
    if (newGenres.length) {
  ...
  private async findOrCreateProductions(productions: string[], creatorId: bigint, session: ClientSession) {
    ...
    for (let i = 0; i < productions.length; i++) {
      if (productions[i].startsWith('create:')) {
        ...
        if (!name) throw new HttpException({ code: StatusCode.IS_NOT_EMPTY, message: 'Production name must not be empty' }, HttpStatus.BAD_REQUEST);
        if (name.length > 150) throw new HttpException({ code: StatusCode.MAX_LENGTH, message: 'Production name must not be longer than 150 characters' }, HttpStatus.BAD_REQUEST);
  ...
  private async findOrCreateTags(tags: string[], creatorId: bigint, session: ClientSession) {
    ...
    for (let i = 0; i < tags.length; i++) {
      if (tags[i].startsWith('create:')) {
        ...
        if (!name) throw new HttpException({ code: StatusCode.IS_NOT_EMPTY, message: 'Tag name must not be empty' }, HttpStatus.BAD_REQUEST);
        if (name.length > 32) throw new HttpException({ code: StatusCode.MAX_LENGTH, message: 'Tag name must not be longer than 32 characters' }, HttpStatus.BAD_REQUEST);
        newTags.push({ name });
      } else {
        try {
    ...
    if (tagCount !== existingTagIds.length) throw new HttpException({ code: StatusCode.TAGS_NOT_FOUND, message: 'Cannot find all the required tags' }, HttpStatus.BAD_REQUEST);
    if (newTags.length) {
  ...
  private async validateSubtitle(file: Storage.MultipartFile) {
    if (!file.fields.language) throw new HttpException({ code: StatusCode.IS_NOT_EMPTY, message: 'Language is required' }, HttpStatus.BAD_REQUEST);
    const language = file.fields.language['value'];
    if (!ISO6391.validate(language)) throw new HttpException({ code: StatusCode.IS_ISO6391, message: 'Language must be an ISO6391 code' }, HttpStatus.BAD_REQUEST);
  ...
  private async createLinkedMediaSource(addMediaSourceDto: AddLinkedMediaSourceDto, mediaId: bigint, episodeId?: bigint) {
    ...
    if (!linkedFile) throw new HttpException({ code: StatusCode.DRIVE_FILE_NOT_FOUND, message: 'Linked file not found' }, HttpStatus.NOT_FOUND);
  ...
  private findReplaceStreams(uploadedSource: FlattenMaps<MediaStorageDocument>, targetVideoCodecs?: number, audioOnly?: boolean, videoOnly?: boolean) {
    ...
    if (!videoOnly && !audioOnly) {
      if (targetVideoCodecs) {
        for (let i = 0; i < STREAM_CODECS.length; i++) {
          if (targetVideoCodecs & STREAM_CODECS[i])
            replaceStreams.push(...videoStreams.filter((s) => s.codec === STREAM_CODECS[i]).map((s) => s._id), ...manifestStreams.filter((s) => s.codec === STREAM_CODECS[i]).map((s) => s._id));
      ...
      if (targetVideoCodecs) {
        for (let i = 0; i < STREAM_CODECS.length; i++) {
          if (targetVideoCodecs & STREAM_CODECS[i]) replaceStreams.push(...videoStreams.filter((s) => s.codec === STREAM_CODECS[i]).map((s) => s._id));
        }
      } else {
  ...
  private async deleteMediaStreamFromStorage(ids: bigint[], sourceId: bigint, storage: ExternalStorage) {
    ...
    await Promise.all(
      ids.map((id) =>
        deleteStreamLimit(() => {
          if (storage.kind === CloudStorage.FILER) return this.filerService.deleteFolder(`${sourceId}/${id}`, storage);
          if (storage.kind === CloudStorage.S3) return this.s3Service.deleteFolder(`${sourceId}/${id}`, storage);
  ...
  private async addMediaChapter(chapters: Types.Array<MediaChapter>, addMediaChapterDto: AddMediaChapterDto) {
    ...
    if (checkChapter) throw new HttpException({ code: StatusCode.CHAPTER_TIME_DUPLICATED, message: 'Duplicated start and end time' }, HttpStatus.CONFLICT);
  ...
  private async validateCollection(id: bigint) {
    ...
    if (!collection) throw new HttpException({ code: StatusCode.COLLECTION_NOT_FOUND, message: 'Collection not found' }, HttpStatus.NOT_FOUND);
    return collection;
  }
  ...
  deleteGenreMedia(genreId: bigint, mediaIds: bigint[], session?: ClientSession) {
    if (mediaIds.length) return this.mediaModel.updateMany({ _id: { $in: mediaIds } }, { $pull: { genres: genreId } }, { session });
  }

  deleteProductionMedia(productionId: bigint, mediaIds: bigint[], session?: ClientSession) {
    if (mediaIds.length) return this.mediaModel.updateMany({ _id: { $in: mediaIds } }, { $pull: { studios: productionId, producers: productionId } }, { session });
  }

  deleteCollectionMedia(collectionId: bigint, mediaIds: bigint[], session?: ClientSession) {
    if (mediaIds.length) return this.mediaModel.updateMany({ _id: { $in: mediaIds } }, { $pull: { inCollections: collectionId } }, { session });
  }

  deleteTagMedia(tagId: bigint, mediaIds: bigint[], session?: ClientSession) {
    if (mediaIds.length) return this.mediaModel.updateMany({ _id: { $in: mediaIds } }, { $pull: { tags: tagId } }, { session });
  }
