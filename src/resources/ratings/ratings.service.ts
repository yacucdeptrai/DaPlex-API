...
export class RatingsService {
  ...
  async create(createRatingDto: CreateRatingDto, authUser: AuthUserDto) {
    ...
    try {
      ...
      if (!updatedMedia) throw new HttpException({ code: StatusCode.MEDIA_NOT_FOUND, message: 'Media not found' }, HttpStatus.NOT_FOUND);
  ...
  async findAll(cursorPageRatingDto: CursorPageRatingsDto, headers: HeadersDto, authUser: AuthUserDto) {
    ...
    if (authUser.isAnonymous && !cursorPageRatingDto.user) throw new HttpException({ code: StatusCode.RATING_USER_NOT_FOUND, message: 'User not found' }, HttpStatus.NOT_FOUND);
  ...
  async remove(id: bigint, authUser: AuthUserDto) {
    ...
    await session
      .withTransaction(async () => {
        ...
        if (!deletedRating) throw new HttpException({ code: StatusCode.RATING_NOT_FOUND, message: 'Rating not found' }, HttpStatus.NOT_FOUND);
