...
export class SettingsService {
  constructor(
    @InjectModel(Setting.name, MongooseConnection.DATABASE_A) private settingModel: Model<SettingDocument>,
  ...
  async create(createSettingDto: CreateSettingDto) {
    ...
    if (check) throw new HttpException({ code: StatusCode.SETTING_EXIST, message: 'Setting has already been created' }, HttpStatus.BAD_REQUEST);
    ...
    await Promise.all([setting.save(), this.auditLogService.createLogFromBuilder(auditLog)]);
  ...
  async findOne(authUser: AuthUserDto) {
    ...
    if (!setting) throw new HttpException({ code: StatusCode.SETTING_NOT_EXIST, message: 'Setting was not created' }, HttpStatus.NOT_FOUND);
    if (authUser.hasPermission) return setting;
  }

  findOneAndCache() {
    return this.localCacheService.wrap<Setting>(
  ...
  async update(updateSettingDto: UpdateSettingDto, authUser: AuthUserDto) {
    if (!Object.keys(updateSettingDto).length) throw new HttpException({ code: StatusCode.EMPTY_BODY, message: 'Nothing to update' }, HttpStatus.BAD_REQUEST);
    ...
    await session
      .withTransaction(async () => {
        if (updateSettingDto.owner != undefined && updateSettingDto.owner !== <any>setting.owner) {
          ...
          if (!newUser || !oldUser) throw new HttpException({ code: StatusCode.USER_NOT_FOUND, message: 'User does not exist' }, HttpStatus.NOT_FOUND);
        ...
        if (updateSourceStorages) setting.mediaSourceStorages = <any>updateSourceStorages;
        ...
        if (updateLinkedSourceStorages) setting.linkedMediaSourceStorages = <any>updateLinkedSourceStorages;
        auditLog.getChangesFrom(setting);
        await Promise.all([setting.save({ session }), this.auditLogService.createLog(authUser._id, setting._id, Setting.name, AuditLogType.SETTINGS_UPDATE)]);
      })
  ...
  async deleteMediaPosterStorage(id: bigint, session: ClientSession) {
    ...
    if (setting) await this.clearMediaPosterCache();
  }
  ...
  async deleteMediaBackdropStorage(id: bigint, session: ClientSession) {
    ...
    if (setting) await this.clearMediaBackdropCache();
  }
  ...
  async deleteMediaSourceStorage(id: bigint, session: ClientSession) {
    ...
    if (setting) await this.clearMediaSourceCache();
  }
  ...
  async deleteMediaSubtitleStorage(id: bigint, session: ClientSession) {
    ...
    if (setting) await this.clearMediaSubtitleCache();
  }
  ...
  async deleteTVEpisodeStillStorage(id: bigint, session: ClientSession) {
    ...
    if (setting) await this.clearTVEpisodeStillCache();
  }
  ...
  async findMediaPosterStorage() {
    return this.localCacheService.wrap<ExternalStorage>(
      CachePrefix.MEDIA_POSTER_STORAGE,
      async () => {
        ...
        if (!storage) throw new HttpException({ code: StatusCode.POSTER_STORAGE_NOT_SET, message: 'Poster storage is not available, please contact the owner to set it up' }, HttpStatus.BAD_REQUEST);
  ...
  async findMediaBackdropStorage() {
    return this.localCacheService.wrap<ExternalStorage>(
      CachePrefix.MEDIA_BACKDROP_STORAGE,
      async () => {
  ...
  async findTVEpisodeStillStorage() {
    return this.localCacheService.wrap<ExternalStorage>(
      CachePrefix.TV_EPISODE_STILL_STORAGE,
      async () => {
        ...
        if (!storage) throw new HttpException({ code: StatusCode.STILL_STORAGE_NOT_SET, message: 'Still storage is not available, please contact the owner to set it up' }, HttpStatus.BAD_REQUEST);
        await this.externalStoragesService.decryptToken(storage);
        return storage;
      },
      3_600_000
  ...
  async findMediaSourceStorage(options: { decrypt?: boolean } = {}) {
    ...
    if (decrypt) await this.externalStoragesService.decryptToken(storage);
    return storage;
  }

  async findLinkedMediaSourceStorages() {
    const setting = await this.settingModel
      .findOne({}, { linkedMediaSourceStorages: 1 })
      .populate({
