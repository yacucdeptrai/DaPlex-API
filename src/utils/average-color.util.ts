// https://github.com/fast-average-color/fast-average-color-node/blob/main/src/index.ts
...
export async function getAverageColor(resource: string | Buffer, options: FastAverageColorOptions = {}): Promise<AverageColorResult> {
  let input = resource;

  /* Only local file path will be used
  ...
  */
  ...
  const colorResult = fac.prepareResult(fac.getColorFromArray4(pixelArray, options));
